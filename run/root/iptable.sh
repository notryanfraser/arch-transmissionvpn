...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	# setup route for transmission webui using set-mark to route traffic for port 9091 to eth0
	echo "9091    webui" >> /etc/iproute2/rt_tables
...
# accept input to transmission Web UI port 9091
iptables -A INPUT -i "${docker_interface}" -p tcp --dport 9091 -j ACCEPT
iptables -A INPUT -i "${docker_interface}" -p tcp --sport 9091 -j ACCEPT
...
for lan_network_item in "${lan_network_list[@]}"; do
	...
	# accept input to transmission daemon port - used for lan access
...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	# accept output from transmission-web port 9091 - used for external access
...
# accept output from transmission-web port 9091 - used for lan access
iptables -A OUTPUT -o "${docker_interface}" -p tcp --dport 9091 -j ACCEPT
iptables -A OUTPUT -o "${docker_interface}" -p tcp --sport 9091 -j ACCEPT
...
for lan_network_item in "${lan_network_list[@]}"; do
	...
	# accept output to transmission daemon port - used for lan access
