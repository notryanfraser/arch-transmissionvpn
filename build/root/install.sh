...
pacman_packages="curl libevent gtk3 qt5-base qt5-tools intltool python2-service-identity python2-mako python2-notify python2-pillow gnu-netcat ipcalc transmission-cli"
...
# create path to store transmission python eggs
...
# set permissions for python eggs to be a more restrictive 755, this prevents the warning message thrown by transmission on startup
...
export TRANSMISSION_DAEMON_LOG_LEVEL=$(echo "${TRANSMISSION_DAEMON_LOG_LEVEL}" | sed -e 's~^[ \t]*~~;s~[ \t]*$~~')
if [[ ! -z "${TRANSMISSION_DAEMON_LOG_LEVEL}" ]]; then
	echo "[info] TRANSMISSION_DAEMON_LOG_LEVEL defined as '${TRANSMISSION_DAEMON_LOG_LEVEL}'" | ts '%Y-%m-%d %H:%M:%.S'
else
	echo "[info] TRANSMISSION_DAEMON_LOG_LEVEL not defined,(via -e TRANSMISSION_DAEMON_LOG_LEVEL), defaulting to 'info'" | ts '%Y-%m-%d %H:%M:%.S'
	export TRANSMISSION_DAEMON_LOG_LEVEL="info"
fi

export TRANSMISSION_WEB_LOG_LEVEL=$(echo "${TRANSMISSION_WEB_LOG_LEVEL}" | sed -e 's~^[ \t]*~~;s~[ \t]*$~~')
if [[ ! -z "${TRANSMISSION_WEB_LOG_LEVEL}" ]]; then
	echo "[info] TRANSMISSION_WEB_LOG_LEVEL defined as '${TRANSMISSION_WEB_LOG_LEVEL}'" | ts '%Y-%m-%d %H:%M:%.S'
else
	echo "[info] TRANSMISSION_WEB_LOG_LEVEL not defined,(via -e TRANSMISSION_WEB_LOG_LEVEL), defaulting to 'info'" | ts '%Y-%m-%d %H:%M:%.S'
	export TRANSMISSION_WEB_LOG_LEVEL="info"
fi
